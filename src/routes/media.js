...
router.get('/', authenticateJwt, requireRole('Contributor'), async (req, res, next) => {
  try { return res.status(200).json(await mediaStore.listMedia()); }
  catch (err) { next(err); }
});
...
router.post('/folders', authenticateJwt, auditLogger, requireRole('Contributor'), async (req, res, next) => {
  try {
    ...
    if (!name || typeof name !== 'string' || !name.trim()) return res.status(400).json({ error: 'Folder name is required.' });
    const folder = await mediaStore.createFolder({ name: name.trim(), parentId: parentId || null, folderColor: folderColor || '#FF0000' });
...
router.post('/upload', authenticateJwt, auditLogger, requireRole('Contributor'), upload.single('file'), async (req, res, next) => {
  try {
    ...
    const now = new Date();
    const storageKey = `media/${now.getUTCFullYear()}/${String(now.getUTCMonth() + 1).padStart(2, '0')}/${uniquePrefix}-${safeName}`;

    const publicUrl = await wasabiService.uploadToWasabi(req.file.buffer, storageKey, req.file.mimetype, {
      throwOnError: true,
      profile: 'media',
      publicRead: false
    });
...
router.use((err, req, res, next) => {
  if (err instanceof multer.MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: `File exceeds the ${MAX_UPLOAD_BYTES / (1024 * 1024)}MB upload limit.` });
    return res.status(400).json({ error: err.message });
  }
  if (err && err.message && err.message.startsWith('Unsupported media type:')) return res.status(415).json({ error: err.message });
  return next(err);
});
