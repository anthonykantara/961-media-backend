...
router.patch('/users/:id', auditLogger, requireRole('Admin'), async (req, res, next) => {
  try {
    ...
    const result = await query(
      `UPDATE users SET ${fields.join(', ')} WHERE id = $${index} RETURNING id, email, role, display_name, identity_provider, identity_subject, is_active, last_login_at, created_at, updated_at`,
      values
    );
...
router.get('/analytics', async (req, res, next) => {
  try {
    ...
    const authors = await query(
      `SELECT COALESCE(author, 'Unknown') AS name,
              COUNT(*)::int AS posts,
              COALESCE(SUM(CASE
                WHEN LOWER(views) ~ 'm$' THEN REPLACE(LOWER(views), 'm', '')::numeric * 1000000
                WHEN LOWER(views) ~ 'k$' THEN REPLACE(LOWER(views), 'k', '')::numeric * 1000
       ...
       FROM articles
       WHERE created_at >= NOW() - ($1::int * INTERVAL '1 day')
       GROUP BY COALESCE(author, 'Unknown') ORDER BY views DESC LIMIT 20`,
      [days]
    );
